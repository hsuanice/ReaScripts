...
local TOKEN_LIST = {
  ...
  "${counter:2}","${counter:3}","$interleave","$interum","$baseindex","$baseidx","$overlapindex","$rangeindex","$chnum",
}
...
local function scan_metadata()
  ...
  status_msg = (total==0) and "No items selected."
            or string.format("Scanned %d item(s). Preview shows first %d.", total, math.min(total, preview_cap))
end
...
local function apply_renaming_step()
  ...
  for i = p.i, i_end do
    ...
    -- Rename：若有空 token 且啟用 Skip，就跳過 rename（Note 照常）
