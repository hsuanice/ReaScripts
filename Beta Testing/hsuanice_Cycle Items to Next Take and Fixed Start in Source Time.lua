...
@about
  ...
  matches the active take before cycling to NEXT take (42349). Handles loop/non-loop bounds.
@note
...
-- ===== User options =====
local DEBUG          = true          -- print console logs
local HONOR_BOUNDS   = true          -- normalize SrcStart: clamp (non-loop) / wrap (loop)
local EPS            = 1e-4          -- seconds; threshold to consider "already aligned"
-- ========================
...
log("=== Cycle NEXT (fix SrcStart by DesTC) | selected=%d ===", N)
    ...
    if not tgt then
      ...
      if changed then
        ...
        if delta <= EPS then
          ...
          log(("[item %d] next: no change (library reported no write), ΔDesTC=%.6f"):format(i+1, delta))
        end
      end
    end
  end
...
-- Now actually cycle to NEXT
r.Main_OnCommand(42349, 0) -- Take: Cycle items to next take
r.UpdateArrange()
r.Undo_EndBlock("Cycle items to NEXT take (fix SrcStart by DesTC)", -1)
