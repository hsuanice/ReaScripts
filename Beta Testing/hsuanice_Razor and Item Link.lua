...
@changelog
  v0.1.4 - English-only comments; no functional changes.
]]
...
reaper.atexit(function()
  ...
  reaper.SetExtState("hsuanice_RazorItemLink", "enabled", "0", true)
end)

-- Mark as enabled for this run (useful if other tools want to read it)
reaper.SetExtState("hsuanice_RazorItemLink", "enabled", "1", true)
...
local function get_razor_ranges(tr)
  ...
  -- Primary parse for EXT format: "start end "GUID" ..." (grab the first two numbers)
  for a, b in s:gmatch("([%d%.]+)%s+([%d%.]+)%s+\"") do
    a, b = tonumber(a), tonumber(b)
    if a and b and b > a then ranges[#ranges+1] = {a, b} end
  end

  -- Fallback parse: pairwise scan of all numeric tokens (start/end pairs)
  if #ranges == 0 then
    local nums = {}
...
-- Add-select items that overlap any of the given ranges on the same track.
-- Overlap rule: itemStart < rangeEnd AND itemEnd > rangeStart.
local function select_items_in_ranges(tr, ranges)
  ...
  for i = 0, icnt-1 do
    ...
    for _, r in ipairs(ranges) do
      if endpos > r[1] and pos < r[2] then
...
local function mainloop()
  ...
  if sig ~= last_sig then
    ...
    for i = 0, tcnt-1 do
      ...
      if #ranges > 0 then
        select_items_in_ranges(tr, ranges)
      end
      -- Tracks without Razor Areas are left untouched (no deselection).
    end
    reaper.PreventUIRefresh(-1)
