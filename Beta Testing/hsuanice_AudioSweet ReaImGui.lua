...
@changelog
  v251107.0240 (0.1.0-beta) - MAJOR: GLUE MODE NOW USES TS-WINDOW WHEN TS EXISTS (CORE MODIFICATION)
    - Changed: GLUE button behavior when Time Selection (TS) exists
      • With TS: Uses TS-Window glue (NO handles, splits at boundaries, non-destructive)
      • Without TS: Uses Units glue (with handles as configured)
      • Take/Track FX and other settings are respected in both paths
    - Changed: AUTO mode maintains original behavior (unchanged)
      • TS ≈ selection span → Units glue with handles
      • TS ≠ selection → TS-Window glue without handles
    - Technical: RGWH Core glue_selection() now auto-detects TS and chooses appropriate path
    - Technical: Pre-glue boundary splitting added to TS-Window path (preserves items outside TS)
    - Technical: Handle extension logic removed from TS-Window path
    - Rationale: Users expect GLUE button to respect TS range without adding handles
