...
@changelog
  v0.7.3 remove BEFORE/AFTER UI and function
  v0.7.2
...
@changelog
  v0.7.3
    - Refactor: Split UI/logic; delegated table selection / copy / paste / export
      to helper library "hsuanice_List Table.lua".
    - Behavior: Shift-rectangle selection, Copy, Paste, and Save (TSV/CSV)
      now strictly follow the current on-screen column order (after header drag).
    - Visibility-aware: With "Show muted items" OFF, Copy/Save operate only on visible rows,
      and Paste writes only into visible rows.
  v0.7.2
    - New: “Show muted items” toggle in the toolbar.
      • When off, rows with Mute=on are hidden from the table.
      • Copy and Save (TSV/CSV) export only the currently visible rows, in the exact on-screen column order.
...
-- === List Table helper (selection/copy/paste/export) ===
local LISTLIB_PATH = reaper.GetResourcePath() .. "/Scripts/hsuanice Scripts/Library/hsuanice_List Table.lua"
local LT = dofile(LISTLIB_PATH)
...
-- Map internal column id -> header label for exports
local function header_label_from_id(col_id)
  if col_id==12 or col_id==13 then
    local sh, eh = TFLib.headers(TIME_MODE, {pattern=CUSTOM_PATTERN})
    return (col_id==12) and sh or eh
  end
  return ({[1]="#",[2]="TrkID",[3]="Track Name",[4]="Take Name",[5]="Item Note",
           [6]="Source File",[7]="Meta Track Name",[8]="Chan#", [9]="Interleave",
           [10]="Mute",[11]="Color"})[col_id] or tostring(col_id)
end
...
local function draw_summary_popup()
  if reaper.ImGui_BeginPopupModal(ctx, POPUP_TITLE, true, reaper.ImGui_WindowFlags_AlwaysAutoResize()) then
    -- 依目前顯示來源挑 raw src，再投影到「可見列」
    local src = ROWS
    if     TABLE_SOURCE == "before" then src = SNAP_BEFORE
    elseif TABLE_SOURCE == "after"  then src = SNAP_AFTER end

    -- 關鍵：Summary 用 get_view_rows()，與表格顯示一致（尊重 Show muted / 欄位顯示）
    local rows = get_view_rows(src)
...
if reaper.ImGui_IsItemHovered(ctx) then
  ...
  if reaper.ImGui_Button(ctx, "Copy (TSV)", 110, 24) then
    ...
    local txt  = LT.build_table_text("tsv", rows, COL_ORDER, header_label_from_id, get_cell_text)
    reaper.ImGui_SetClipboardText(ctx, txt)
  end
  ...
  reaper.ImGui_SameLine(ctx)
  if reaper.ImGui_Button(ctx, "Save .tsv", 100, 24) then
    local p = choose_save_path(
      "ItemList_"..timestamp()..".tsv",
      "Tab-separated (*.tsv)\0*.tsv\0All (*.*)\0*.*\0"
    )
    if p then
      local rows = get_view_rows()
      local txt  = LT.build_table_text("tsv", rows, COL_ORDER, header_label_from_id, get_cell_text)
      write_text_file(p, txt)
    end
  end
  ...
  if reaper.ImGui_Button(ctx, "Save .csv", 100, 24) then
    local p = choose_save_path(
      "ItemList_"..timestamp()..".csv",
      "CSV (*.csv)\0*.csv\0All (*.*)\0*.*\0"
    )
    if p then
      local rows = get_view_rows()
      local txt  = LT.build_table_text("csv", rows, COL_ORDER, header_label_from_id, get_cell_text)
      write_text_file(p, txt)
    end
  end
...
local function draw_table(rows, height)
  ...
  if reaper.ImGui_BeginTable(ctx, "live_table", 13, flags, -FLT_MIN, height or 360) then
    ...
    local function handle_cell_click(guid, col)
      ...
      -- 注意：這裡的 rows 和 row_index_map 已經從 draw_table() 外層傳進來，
      -- 所以不用再重新算一次 get_view_rows()。

      if m.shift and SEL.anchor then
        -- 用小庫做矩形選（跟隨目前表頭拖曳後的視覺欄序）
        LT.sel_rect_apply(
          rows, row_index_map,
          SEL.anchor.guid, guid,
          SEL.anchor.col,  col,
...
local function loop()
  ...
  -- Clipboard shortcuts (when NOT in InputText editing)
  if not (EDIT and EDIT.col) then
    -- Copy selection (any view) — use LT to follow visual column order & visible rows
    if shortcut_pressed(reaper.ImGui_Key_C()) then
      local src  = rows_for_current_view()
      local rows = get_view_rows(src)
      local rim  = LT.build_row_index_map(rows)
      local tsv  = LT.copy_selection(rows, rim, sel_has, COL_ORDER, COL_POS, get_cell_text)
      reaper.ImGui_SetClipboardText(ctx, tsv)
    end

    -- 解析剪貼簿 → 用小庫轉表、扁平化、取得形狀
    local clip = reaper.ImGui_GetClipboardText(ctx) or ""
