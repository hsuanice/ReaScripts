...
--
--   Shifts the content (start offset) of selected audio items later
--   by 1 grid division. Item position and length remain unchanged.
--   Clamped so content does not exceed source file end.
--
--   - Mac shortcut (PT) : Command + Numpad Plus
--   - Tags              : Editing
--
-- @changelog
--   0.1.1 [260416.1336]
...
if sel_count == 0 then
  r.ShowMessageBox("No items selected.", "Nudge Clip Contents Later By Grid", 0)
  return
end
...
for i = 0, sel_count - 1 do
  ...
  if take and not r.TakeIsMIDI(take) then
    ...
    local rate    = math.max(r.GetMediaItemTakeInfo_Value(take, "D_PLAYRATE"), 1e-9)
    local src     = r.GetMediaItemTake_Source(take)
    local src_len = r.GetMediaSourceLength(src)
    local max_offs = math.max(0, src_len - len * rate)
    r.SetMediaItemTakeInfo_Value(take, "D_STARTOFFS", math.min(offs + grid_sz * rate, max_offs))
  end
end
