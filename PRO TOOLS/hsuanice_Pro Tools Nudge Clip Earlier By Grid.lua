...
--   1. Items selected    → nudge items left by 1 grid, time selection follows
--   2. Razor area exists → nudge all razor areas left by 1 grid
--   3. Neither           → move edit cursor left by 1 grid (fallback)
--
...
local function nudge_razor_areas(areas, delta)
  -- Group by track, rebuild razor string
  local by_track = {}
...
if has_items then
  for i = 0, sel_count - 1 do
    ...
    r.SetMediaItemInfo_Value(item, "D_POSITION", math.max(0, pos - grid_sz))
  end
...
-- 2. Nudge razor areas
if has_razor then
  -- Use grid size at the start of the first razor area
