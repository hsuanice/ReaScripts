...
@changelog
  ...
  v260718.1918
  - Cache invalidation: bumped CACHE_VERSION to 2.1 after Metadata Read v0.3.1
    interleave/TRACK_LIST resolution changes.
    • Prevents Item List scripts from reusing stale meta_trk_name/channel_num
      generated by older read logic.

  v260711.1929
  - Fix: Cache invalidation now detects active-take switches on the same item.
    • Item mod hash now includes active take index (I_CURTAKE).
    • Hash also includes take playback-defining params (D_STARTOFFS, D_PLAYRATE,
      I_CHANMODE) to avoid stale metadata when take context changes.
    • Resolves cases where metadata stayed stale until manual Clear Cache.
...
local function get_item_mod_hash(item)
  ...
  local src_hash = 0
  local start_offs_hash = 0
  local playrate_hash = 0
  local chanmode_hash = 0
  local src_fn = ""
  if tk then
    ...
    local start_offs = reaper.GetMediaItemTakeInfo_Value(tk, "D_STARTOFFS") or 0
    local playrate = reaper.GetMediaItemTakeInfo_Value(tk, "D_PLAYRATE") or 1
    local chanmode = reaper.GetMediaItemTakeInfo_Value(tk, "I_CHANMODE") or 0
    start_offs_hash = math.floor(start_offs * 1000)
    playrate_hash = math.floor(playrate * 1000000)
    chanmode_hash = math.floor(chanmode)
  end
