--[[
@description hsuanice_Razor and Item Link like Pro Tools
@version 1.0.0
@author hsuanice
@about
  Single-button toggle. When ON, it add-selects media items ONLY if a Razor Area on the same track
  FULLY contains the item (itemStart >= areaStart AND itemEnd <= areaEnd). If a Razor Area covers
  only part of an item, that item will NOT be selected. When OFF, it stops watching.
  - Coexists with Razor Areas (never modifies them).
  - Add-only by default (does not clear existing selection). Optionally enable strict sync below.
  - No ReaScript task-control popup: relaunching this action auto-terminates the previous instance
    and keeps the toolbar state in sync.

  This script was generated using ChatGPT based on design concepts and iterative testing by hsuanice.
  hsuanice served as the workflow designer, tester, and integrator for this tool.
@changelog
  v1.0.0 - Pro Tools behavior (full containment required). Toolbar-friendly true toggle.
]]
